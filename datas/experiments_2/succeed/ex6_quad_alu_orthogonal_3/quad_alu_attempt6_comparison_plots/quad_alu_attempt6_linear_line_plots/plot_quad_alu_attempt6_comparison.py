...
def plot_selected_slices(quad_values, alu_values, grids, pass_grids, output_dir):
    """Plot total and active per-pass GPU times on the two selected slice figures.

    Renderer identity is encoded by color and marker. Total time uses a thick
    solid line; individual passes reuse the renderer color with thinner dashed
    lines. Forward's only active pass is equal to its total and therefore lies
    directly on top of the total curve.
    """
    image_files = []
    ...
    def plot_variant_and_passes(ax, x_values, variant, total_values, pass_value_getter):
        ...
        for pass_name, column, linestyle in rule["passes"]:
            pass_values = pass_value_getter(column)
            ax.plot(
                x_values,
                pass_values,
                linestyle=linestyle,
    ...
    fig.suptitle("Quad Waste × ALU Orthogonal — ALU-axis Slices with Pass Times", fontsize=15)
    fig.text(
        0.5,
        0.006,
    ...
    image_files.extend(save_figure(fig, output_dir, "09_selected_alu_axis_slices_with_pass_times"))
    ...
    fig.suptitle("Quad Waste × ALU Orthogonal — Waste-quad-axis Slices with Pass Times", fontsize=15)
    fig.text(
        0.5,
        0.006,
        "Thick solid + marker: total · Thin dashed/dotted: individual pass · Forward pass coincides with Forward total",
    ...
    image_files.extend(save_figure(fig, output_dir, "10_selected_quad_axis_slices_with_pass_times"))
    return image_files
...
def write_summary(
    ...
    lines.extend(
        [
            ...
            "- 09: selected ALU-axis slices with total and per-pass times",
            "- 10: selected waste-quad-axis slices with total and per-pass times",
...
def main():
    ...
    image_files.extend(plot_selected_slices(quad_values, alu_values, grids, pass_grids, output_dir))
