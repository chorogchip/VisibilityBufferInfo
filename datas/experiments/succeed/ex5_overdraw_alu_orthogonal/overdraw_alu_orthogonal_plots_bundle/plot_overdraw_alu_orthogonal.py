#!/usr/bin/env python3
"""
Plot an overdraw_alu_orthogonal TVBPerf CSV.

The experiment sweeps two numeric dimensions (overdraw and ALU calculation
...
10.   Selected overdraw-axis slices
11.   Physics-guided interaction-model fit diagnostics

It also exports validation reports, winner/crossover tables, empirical model
coefficients, PNG/SVG versions, and a ZIP bundle.

Usage:
    python plot_overdraw_alu_orthogonal.py overdraw_alu_orthogonal.csv \
        --output-dir overdraw_alu_orthogonal_plots
"""
...
def load_csv(csv_path: Path):
    ...
    overdraw_col = resolve_column(
        fieldnames,
        ["variable-overdraw-count", "overdraw-count", "param_overdraw_count"],
    )
    ...
    for row in success_raw:
        ...
        overdraw = as_float(row.get(overdraw_col))
        alu = as_float(row.get(alu_col))
        if not all(math.isfinite(value) for value in (variant_value, overdraw, alu)):
            continue
        variant = int(variant_value)
        if variant not in VARIANT_RULES or overdraw < 0 or alu < 0:
            continue
...
def aggregate(rows):
    ...
    for row in rows:
        key = (row["variant"], row["overdraw"], row["alu"])
        buckets.setdefault(key, []).append(row)

    aggregated = []
    for (variant, overdraw, alu), group in sorted(buckets.items()):
        item = {
            "variant": variant,
...
def build_grid(rows):
    overdraw_values = sorted({row["overdraw"] for row in rows})
    alu_values = sorted({row["alu"] for row in rows})
    overdraw_index = {value: index for index, value in enumerate(overdraw_values)}
    ...
    for variant in VARIANT_ORDER:
        total_grid = np.full((len(overdraw_values), len(alu_values)), np.nan)
        ...
        for row in rows:
            if row["variant"] != variant:
                continue
            oi = overdraw_index[row["overdraw"]]
    ...
    return np.array(overdraw_values), np.array(alu_values), grids, pass_grids, repeat_grid
...
def apply_sampled_axis_style(ax, overdraw_values, alu_values):
    ...
    ax.set_ylim(float(overdraw_values.min()), float(overdraw_values.max()))
    ax.set_xlabel("ALU calculation count")
    ax.set_ylabel("Overdraw count")
...
def apply_index_axis_style(ax, overdraw_values, alu_values):
    ax.set_xticks(np.arange(len(alu_values)))
    ax.set_xticklabels([safe_number(value) for value in alu_values])
    ax.set_yticks(np.arange(len(overdraw_values)))
    ax.set_yticklabels([safe_number(value) for value in overdraw_values])
    ax.set_xlabel("ALU calculation count")
    ax.set_ylabel("Overdraw count")
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)
...
def plot_variant_contours(overdraw_values, alu_values, grids, output_dir):
    image_files = []
    X, Y = np.meshgrid(alu_values, overdraw_values)
    for number, variant in enumerate(VARIANT_ORDER, start=1):
        ...
        apply_sampled_axis_style(ax, overdraw_values, alu_values)
        ax.set_title(
            f"Overdraw × ALU Orthogonal — Variant {variant} · {rule['renderer']} · Total Time"
        )
...
def plot_winner_map(overdraw_values, alu_values, grids, output_dir):
    ...
    apply_index_axis_style(ax, overdraw_values, alu_values)
    ax.set_title(
        "Overdraw × ALU Orthogonal — Fastest Renderer Winner Map\n"
...
def plot_winner_margin(overdraw_values, alu_values, grids, output_dir):
    ...
    apply_index_axis_style(ax, overdraw_values, alu_values)
    ax.set_title("Overdraw × ALU Orthogonal — Winner Margin (Second-best / Best)")
...
def plot_regret_maps(overdraw_values, alu_values, grids, output_dir):
    ...
    fig.suptitle("Overdraw × ALU Orthogonal — Cost Relative to the Fastest Renderer", fontsize=15)
...
def plot_speedup_vs_forward(overdraw_values, alu_values, grids, output_dir):
    forward = grids[1]
    variants = [2, 4]
    speedups = [
    ...
    fig.suptitle("Overdraw × ALU Orthogonal — Forward Time / Alternative Time", fontsize=15)
...
def plot_visbuf_vs_prepass(overdraw_values, alu_values, grids, output_dir):
    ...
    apply_sampled_axis_style(ax, overdraw_values, alu_values)
    ax.set_title("Overdraw × ALU Orthogonal — VisBuf vs ForwardPrepass Crossover")
    colorbar = fig.colorbar(contour, ax=ax, pad=0.02)
...
def plot_selected_slices(overdraw_values, alu_values, grids, output_dir):
    image_files = []
    overdraw_indices = nearest_indices(overdraw_values, [0, 2, 12, 32])
    fig, axes = plt.subplots(2, 2, figsize=(17.5, 11.0), constrained_layout=True, sharex=True)
    for ax, oi in zip(axes.flat, overdraw_indices):
        for variant in VARIANT_ORDER:
            rule = VARIANT_RULES[variant]
            ax.plot(
                alu_values,
        ...
        ax.set_title(f"Overdraw = {safe_number(overdraw_values[oi])}")
    ...
    fig.suptitle("Overdraw × ALU Orthogonal — ALU-axis Slices", fontsize=15)
    ...
    fig.suptitle("Overdraw × ALU Orthogonal — Overdraw-axis Slices", fontsize=15)
    image_files.extend(save_figure(fig, output_dir, "10_selected_overdraw_axis_slices"))
    return image_files


def fit_interaction_models(overdraw_values, alu_values, grids):
    O, A = np.meshgrid(overdraw_values, alu_values, indexing="ij")
    ...
    for variant in VARIANT_ORDER:
        ...
        models[variant] = {
            ...
            "prediction_grid": prediction.reshape(O.shape),
            "r_squared": r_squared,
            "rmse_ms": rmse,
            "mape_percent": mape,
        }
...
def plot_model_diagnostics(grids, models, output_dir):
    ...
    fig.suptitle(
        "Physics-guided Interaction Model — T = β₀ + βO·O + βA·A + βOA·O·A",
        fontsize=15,
    )
    return save_figure(fig, output_dir, "11_interaction_model_measured_vs_predicted")
...
def export_winner_tables(overdraw_values, alu_values, grids, output_dir):
    ...
    write_csv(output_dir / "crossover_summary_by_overdraw.csv", crossover_rows)
    return winner_rows, crossover_rows


def export_model_tables(models, output_dir):
    rows = []
    feature_names = ["beta_0_ms", "beta_overdraw_ms_per_count", "beta_alu_ms_per_count", "beta_overdraw_x_alu_ms_per_product"]
    for variant in VARIANT_ORDER:
...
def write_summary(
    ...
    lines = [
        "Overdraw × ALU Orthogonal Plot Generation Summary",
        "==================================================",
        ...
        f"Overdraw samples ({len(overdraw_values)}): {[safe_number(v) for v in overdraw_values]}",
        ...
        "",
        "Winner counts over the sampled 10 × 11 grid:",
    ]
    ...
    lines.extend(
        [
            ...
            "T = β0 + βO·Overdraw + βA·ALU + βOA·Overdraw·ALU",
        ]
    )
    ...
    lines.extend(
        [
            ...
            "- 10: selected overdraw-axis slices",
            "- 11: interaction-model measured-versus-predicted diagnostics",
            "",
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("overdraw_alu_orthogonal_plots"),
    )
    ...
    overdraw_values, alu_values, grids, _, repeat_grid = build_grid(rows)

    expected = len(overdraw_values) * len(alu_values)
    for variant in VARIANT_ORDER:
    ...
    export_winner_tables(overdraw_values, alu_values, grids, output_dir)
    models = fit_interaction_models(overdraw_values, alu_values, grids)
    model_rows = export_model_tables(models, output_dir)

    image_files = []
    image_files.extend(plot_variant_contours(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_winner_map(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_winner_margin(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_regret_maps(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_speedup_vs_forward(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_visbuf_vs_prepass(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_selected_slices(overdraw_values, alu_values, grids, output_dir))
    image_files.extend(plot_model_diagnostics(grids, models, output_dir))
    ...
    write_summary(
        ...
        rows=rows,
        overdraw_values=overdraw_values,
    ...
    zip_path = create_zip(output_dir, csv_path, Path(__file__).resolve())
